...
@Component
public class PdfHtmlConstanciaTPC extends AbstractPdfHtml{
    @Autowired
    ...
    protected void buildPdfDocument(Map<String, Object> model, Document documentPdf, PdfWriter writer, HttpServletRequest hsr, HttpServletResponse response) throws Exception {
        ...
        writer.setPageEvent(new PdfHtmlConstanciaTPC.FooterPageEvent());
    ...
    private static class FooterPageEvent extends PdfPageEventHelper {
        ...
        public FooterPageEvent() {
            try {
                backgroundImage = Image.getInstance(this.getClass().getResource(GlobalConstantine.PDF_IMG + backgroundPage));
                backgroundImage.scaleToFit(300, 300);
            } catch (Exception e) {
                throw new RuntimeException("No se pudo cargar la imagen de fondo", e);
            }
        }
        ...
        public void onEndPage(PdfWriter writer, Document document) {
            ...
            float x = document.right() - backgroundImage.getScaledWidth();
            float y = document.top() - backgroundImage.getScaledHeight();
            ...
            PdfContentByte textCanvas = writer.getDirectContent();
            String footerText = "Teléfonos: 614-7800 anexo 138 y 468 - Email: estudios@lamolina.edu.pe";
            String pageNumberText = "Pág. " + writer.getPageNumber();

            float footerX = (document.right() + document.left()) / 2;
            float footerY = document.bottom() - 15;

            float pageNumberX = document.right();
            float pageNumberY = document.top() + 10;

            try {
                textCanvas.beginText();
                BaseFont baseFont = BaseFont.createFont(BaseFont.HELVETICA, BaseFont.WINANSI, BaseFont.NOT_EMBEDDED);
                textCanvas.setFontAndSize(baseFont, 8);
                textCanvas.showTextAligned(Element.ALIGN_CENTER, footerText, footerX, footerY, 0);
                textCanvas.showTextAligned(Element.ALIGN_RIGHT, pageNumberText, pageNumberX, pageNumberY, 0);
