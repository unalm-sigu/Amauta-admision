...
public class PersonaDAOH extends AbstractEasyDAO<Persona> implements PersonaDAO {
    ...
    @Override
    public List<Persona> allByApellidos(Persona persona) {
        Octavia sql = Octavia.query()
                .from(Persona.class, "per")
                .leftJoin("tipoDocumento td");
        if (!StringUtils.isEmpty(persona.getPaterno())) {
    ...
    @Override
    public void updateColumns(Persona persona, String... columns) {
        Octavia sql = Octavia.update(Persona.class, "per");
        for (String column : columns) {
            sql.set(persona, column);
        }
        this.update(sql);
    }
