...
import pe.edu.lamolina.amauta.controller.academico.calculonotas.CalculoNotasService;
import pe.edu.lamolina.amauta.controller.academico.promedio.ContadorComponent;
import pe.edu.lamolina.amauta.controller.academico.promedio.PromedioSegundoService;
...
public class TestServiceImp implements TestService {
    ...
    private final ContadorComponent contadorComponent;
    private final PromedioSegundoService promedioSegundoService;
...
//    }
    ...
    @Override
    public void loadDataSendDataHistorial(List<Alumno> alumnos, List<MatriculaResumen> resumenesAll, String token, CicloAcademico cicloAcademico) {
        List<MatriculaResumen> resumenex = matriculaResumenDAO.allMatriculadosByCiclo(cicloAcademico);
        //List<Alumno> alumnox = resumenex.stream().map(x -> x.getAlumno()).collect(Collectors.toList());

        for (MatriculaResumen resumene : resumenex) {
            resumenesAll.add(resumene);
        }
        List<Alumno> alumnoz = alumnoDAO.allInfoByAlumnos(resumenex.stream().map(x -> x.getAlumno()).collect(Collectors.toList()));
        for (Alumno alumno : alumnoz) {
            alumnos.add(alumno);
        }
    ...
    @Override
    public CicloAcademico findCiclo(String codigo, Long modalidad) {
        CicloAcademico cicloAcademico = cicloAcademicoDAO.findByCodigoModalidadEstudio(codigo, new ModalidadEstudio(modalidad));
    ...
    @Async
    @Override
    public void trasladarInformcionPromedioForHistorialCiclo(
            DataSessionPivot ds,
            List<Alumno> alumnos,
            List<MatriculaResumen> resumenesAll,
            String token,
            CicloAcademico cicloAcademico) {

        List<MatriculaCurso> cursosMatriculadosAll = matriculaCursoDAO.allActivosByCiclo(cicloAcademico);
        List<MatriculaSeccion> matriculaSeccionAll = matriculaSeccionDAO.allActivesByMatriculaResumen(resumenesAll);
        List<AlumnoCicloCurso> cursosLlevadosAll = alumnoCicloCursoDAO.allByAlumnos(alumnos);

        Map<Long, List<MatriculaSeccion>> mapMatriculaSeccion = TypesUtil.convertListToMapList("matriculaResumen.alumno.id", matriculaSeccionAll);
        Map<Long, List<MatriculaCurso>> mapCursoMatriculado = TypesUtil.convertListToMapList("matriculaResumen.alumno.id", cursosMatriculadosAll);
        Map<Long, List<AlumnoCicloCurso>> mapCursoLlevado = TypesUtil.convertListToMapList("alumnoCiclo.alumno.id", cursosLlevadosAll);
        ...
        for (MatriculaResumen matriculaResumen : resumenesAll) {
            ...
            List<MatriculaSeccion> matriculaSeccion = TypesUtil.getListNotNull(mapMatriculaSeccion.get(alumno.getId()));
            List<MatriculaCurso> cursosMatriculados = TypesUtil.getListNotNull(mapCursoMatriculado.get(alumno.getId()));
            Map<Long, MatriculaCurso> mapMatriculaCurso = TypesUtil.convertListToMap("curso.id", cursosMatriculados);
            List<AlumnoCicloCurso> cursosLlevados = TypesUtil.getListNotNull(mapCursoLlevado.get(alumno.getId()));

            List<MatriculaCurso> cursosActasCerradas = new ArrayList();
            for (MatriculaSeccion matriculaSecc : matriculaSeccion) {
                Curso curso = matriculaSecc.getSeccion().getGrupoSeccion().getCurso();
                if (mapMatriculaCurso.get(curso.getId()) != null && matriculaSecc.getSeccion().getGrupoSeccion().getEstadoGrupoEnum() == EstadoGrupoSeccionEnum.CER) {
                    cursosActasCerradas.add(mapMatriculaCurso.get(curso.getId()));
                } else {
                    System.out.println("Curso no cerrado" + curso.getCodigo());
                }
            }

            promedioService.actasNotasHaciaHistorial(matriculaResumen, cursosActasCerradas, cursosLlevados, ds, tokenHisto);
        }
    }
    ...
    public void promediarciclocod(String cicloCod, DataSessionPivot ds) {
        ...
        log.info("ciclos encontrados {}", String.join(",", String.join(",", ciclosStr)));
        ...
        for (CicloAcademico cicloAcademico : ciclos) {
            ...
            log.info("matriculas resumen encontradas {}, del ciclo {}", matriculasResumen.size(), cicloAcademico.toString());
    ...
    public void promediarfull(DataSessionPivot ds, ModalidadEstudioEnum modalidadEnum) {
        ...
        for (String year : allYears) {
            ...
            log.info("Año {}, Alumnos {}, Acumulados {}", year, alumnos.size(), contadorComponent.getCantidadAcumulada());
    ...
    public void promediarAll(Long cicloId, DataSessionPivot ds) {
        ...
        List<MatriculaResumen> matriculasResumen = matriculaResumenDAO.allByCicloFull(cicloAcademico);
        log.info("matriculas resumen encontradas {}", matriculasResumen.size());
...
//    }
    ...
    public void promediarfullBySituacion(String sit, DataSessionPivot ds, ModalidadEstudioEnum modalidadEstudioEnum) {
        ...
        List<ObtencionGrado> graduadosAcumulados = new ArrayList();
        for (String year : allYears) {
            ...
            log.info("Año {}, Alumnos {}, Acumulados {}", year, alumnos.size(), alumnosAcumulados.size());
            List<Egresado> egresados = egresadoDAO.allByAlumnosAceptados(alumnos);
            egresadosAcumulados.addAll(egresados);

            List<ObtencionGrado> graduados = obtencionGradoDAO.allAceptadosByAlumnos(alumnos);
            graduadosAcumulados.addAll(graduados);
        }
        ...
        List<Reincorporacion> reincorporaciones = reincorporadosService.allReincorporacionesByCicloActivo(alumnosAcumulados, ciclosActivos);
        Map<Long, List<Reincorporacion>> mapReincorporacion = TypesUtil.convertListToMapList("alumno.id", reincorporaciones);

        Map<Long, Egresado> mapEgresado = TypesUtil.convertListToMap("alumno.id", egresadosAcumulados);
        Map<Long, ObtencionGrado> mapGraduado = TypesUtil.convertListToMap("alumno.id", graduadosAcumulados);
...
//        Map<Long, List<AlumnoCicloCurso>> mapAlumnoCicloCursoByAlumCi = TypesUtil.convertListToMapList("alumnoCiclo.id", alumnoCicloCurso);
        for (CicloAcademico cicloAcademicoEach : ciclos) {
            log.debug("CICLO: {} - {}", cicloAcademicoEach.getId(), cicloAcademicoEach.getCodigo());
    ...
    public void trasladarMatriculaCursoForPromediosReview(DataSessionPivot ds, String codCiclo) {
        ...
        log.debug("COMENZAMOOOOOOOOOOOO.........");
        log.info("COMENZAMOOOOOOOOOOOO......... iNFO");
        log.trace("COMENZAMOOOOOOOOOOOO......... TRACE");
        ...
        for (CicloAcademico ciclo : ciclos) {
            ...
            log.debug("MAtricula Resumen...");
            List<MatriculaCurso> matriculasCurso = matriculaCursoDAO.allByCiclosFull(ci);
            log.debug("MAtricula Curso...");
            List<MatriculaSeccion> matriculasSeccion = matriculaSeccionDAO.allByMatriculaResumenes(matriculasResumenes);
            log.debug("MAtricula Seccion...");
//        Map<Long, List<MatriculaResumen>> mapMatriculaResumen = TypesUtil.convertListToMapList("cicloAcademico.id", matriculasResumenes);
            ...
            log.debug("Alumno Ciclo Curso...");
            Map<Long, List<AlumnoCicloCurso>> mapAlumnoCicloCursoByAlumCi = TypesUtil.convertListToMapList("alumnoCiclo.id", alumnoCicloCurso);

            SituacionAcademica situacionAcademicaComodin = situacionAcademicaDAO.findByCodigo(SituacionAcademicaEnum.S_00.getValue());
            log.debug("Situaciones ...");
            ...
            for (Alumno alumnoo : alumnoos) {
                log.debug("Alumno {} {}", alumnoo.getCodigo(), alumnoo.getId());
                ...
                log.debug("avance {} de {}", i, alumnoos.size());
                i++;
    ...
    private void trasladarMatriculaCursoForPromediosAlumnoTest(CicloAcademico cicloAcademico,
        ...
        log.debug("##################Ciclo padre {} {} {} alumno {} id {}", cicloAcademico.getId(), cicloAcademico.getYear(), cicloAcademico.getNumeroCiclo(), matriculaResumen.getAlumno().getCodigo(), matriculaResumen.getAlumno().getId());
        if (retiroCiclo != null) {
