...
@Controller
@RequestMapping("academico/tramiteacademico/readmision")
public class ReadmisionController {
    ...
    public String index(Model model) {
        ...
        model.addAttribute("ciclos", JaneHelper.from(service.getCiclosSeis()).only("id,descripcion,codigo")
                .array().toString());

        return "academico/tramitescademicos/readmision/readmision";
    ...
    public JsonResponse save(@RequestBody Readmision readmision, HttpSession session) {
        JsonResponse response = new JsonResponse();
        try {

            DataSessionPivot ds = (DataSessionPivot) session.getAttribute(GlobalConstantine.SESSION_USUARIO);
            service.save(readmision, ds);
    ...
    @ResponseBody
    @RequestMapping(value = "{idReadmision}/anular", method = RequestMethod.GET)
    public JsonResponse anular(@PathVariable Long idReadmision, HttpSession session) {

        JsonResponse response = new JsonResponse();
        try {

            DataSessionPivot ds = (DataSessionPivot) session.getAttribute(GlobalConstantine.SESSION_USUARIO);
            service.anular(idReadmision, ds);
    ...
    public ModelAndView bachillerReporte(Model model, HttpSession session, HttpServletResponse response, @PathVariable Long idReadmision) {
        ...
        DataSessionPivot ds = (DataSessionPivot) session.getAttribute(GlobalConstantine.SESSION_USUARIO);
        service.reporte(model, idReadmision, ds);
