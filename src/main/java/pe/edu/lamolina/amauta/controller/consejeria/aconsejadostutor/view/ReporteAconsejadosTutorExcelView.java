...
import pe.edu.lamolina.amauta.controller.consejeria.consejeros.view.*;
...
@Component
public class ReporteAconsejadosTutorExcelView extends AbstractView {
...
// new XSSFWorkbook(formato);
    ...
    private void createBody(Workbook wb, Sheet sheet, Map<String, Object> model, DataSessionPivot ds) {
        ...
        List<AlumnoConsejero> alumnosConsejero = (List<AlumnoConsejero>) model.get("alumnosTutor");
...
//        sheet.setColumnWidth((column - 1), 35 * 256);
        ...
        excelUtil.replaceVal(irow - 1, column++, "ESTADO MATRICULA", headerCell);
        ...
        int item = 1;
        for (AlumnoConsejero alumnoConsejero : alumnosByConsejero) {
            Alumno alumno = alumnoConsejero.getAlumno();
            column = 0;
            excelUtil.replaceVal(irow, column++, item, estiloNumero);
...
//            excelUtil.replaceVal(irow, column++, consejero.getColaborador().getPersona().getApellidosNombres(), estiloGeneral);
            ...
            excelUtil.replaceVal(irow, column++, alumnoConsejero.getEstadoMatriculableEnum().getValue(), estiloGeneral);
            irow++;
            item++;
        }
