<template>
    <transition name="modal">
        ...
        <div v-bind:id="id" v-bind:data-keyboard="dataKeyboard" v-bind:data-backdrop="dataBackdrop"
             class="modal fade" tabindex="-1" role="dialog">
            <div class="modal-dialog" 
                 v-bind:class="[modalsize, { 'modal-dialog-scrollable' : hasScroll}, { 'modal-dialog-centered' : verticalMiddle}]" 
                 role="document">

                <div class="modal-content" v-bind:style="styleModal">
                    <div v-if="header" class="modal-header"
                         v-bind:style="showBody? '' : 'opacity: 0.5;' ">
                        <slot name="header">
                            <h4 class="modal-title">{{ title }}</h4>
                            <button v-if="btnclose" v-on:click="close" type="button" class="close hide">
...
<script type="text/javascript">
    ...
    module.exports = {
        props: {
            ...
            cancelclass: {type: String, default: 'btn-link'},
            ...
            dataBackdrop: {type: String, default: 'static'},
            dataKeyboard: {type: String, default: 'true'},
        ...
        methods: {
            ...
            blockBody() {
                let $vue = this;
                let body = $($vue.$el).find("#div-body-modal");
                let overlap = $($vue.$el).find("#over-body-modal");
            ...
            unlockBody() {
                let $vue = this;
                let div2 = $($vue.$el).find("#over-body-modal");
                div2.addClass("hide");
            },
            beginProcessing() {
                let $vue = this;
