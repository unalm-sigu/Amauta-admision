<template>
    <div class="panel-body">
        <button class="btn btn-primary btn-sm pull-right m-b-xs" v-on:click="openModal()"> + Agregar Académico</button>
        ...
        <table class="table table-striped">
            ...
            <tbody v-for="(item, index) in $store.state.escalafon.academicoEscalafon" class="editor">
                <tr>
                    <td>
                        {{ item.gradoEnum.descripcion  }}
                    </td>
                    <td>
        ...
        <academico-form ref="academicoFormVUE"></academico-form>
        <confirmar-escalafon-form ref="confirmarEscalafonFormVUE"></confirmar-escalafon-form>
    </div>
</template>

<script>
    const AcademicoForm = httpVueLoader(package + '/academico/AcademicoEscalafonForm.vue');
    const ConfirmarEscalafonForm = httpVueLoader(package + '/ConfirmarEscalafonForm.vue');

    module.exports = {
        components: {AcademicoForm, ConfirmarEscalafonForm},
        data() {
            return{
        ...
        methods: {
            openModal(item) {
                ...
                this.$refs.academicoFormVUE.open(itemClone);
            },
            openModalConfirmacion(item) {
                let tipo = "AcademicoEsc";
                this.$refs.confirmarEscalafonFormVUE.open(item.id, tipo);
            },
            loadList() {
                let $vue = this;
                axios.post($vue.rutaModulo + "/loadListAcademicoEscalafon", $vue.escalafon)
                        .then(function (response) {
                            if (response.data.success) {
                                $vue.setListAcademicoEscalafon(response);
                            }
                        });
            },
            eliminar(item, index) {
            ...
            setListAcademicoEscalafon(item) {
                this.$store.commit('SET_LIST_ACADEMICO', item.data.data);
            }
        }
    };
</script>
