...
var app = new Vue({
    ...
    data: {
        URL_TRAMITES: APP.url('academico/tramiteacademico/docenteResolucion/list'),
    ...
    methods: {
        ...
        customLabelCiclo: function(option) {

            // console.log(option);
            if (option && option.descripcion) {
                if (option.modalidadEstudio && option.modalidadEstudio.codigo) {
                    return option.descripcion + ' ' + option.modalidadEstudio.codigo;
                }
        ...
        loadDocente(nombre) {
            ...
            if (nombre) {
                ...
                $.ajax({
                    url: APP.url("academico/tramiteacademico/docenteResolucion/allDocenteByNombre"),
                    dataType: 'json',
                    type: 'post',
                    data: {nombre: nombre}
                }).then(response => {
        ...
        saveResolucion() {
            ...
            const url = $vue.isEditing
                ...
                : "/academico/tramiteacademico/docenteResolucion/save";

            axios_.post(url, $vue.docenteResolucion)
                .then(response => {
                    ...
                    notify('Error al guardar: ' + (error.response?.data?.message || error.message || 'Error desconocido'), 'error');
                    $vue.$refs.modalNuevo.stop();
                });
        },
        ...
        editar(item) {
            ...
            console.log('¿Hay docenteResolucion?:', item.docenteResolucion);
        ...
        anularDocenteResolucion(item) {
            ...
            swal({
                ...
                if (willDelete) {
                    axios_.get(APP.url('academico/tramiteacademico/docenteResolucion/anular/' + item.tramite.id)).
                    then(({data}) => {
                        notify(data, 'info');
                        $vue.$refs.load.loadRemoteData();
                    }, () => {
        ...
        anular(item) {
            let $vue = this;
            $vue.docenteResolucion = {...item};
            $vue.$refs.modalEliminarTramite.open();
        },
        anularActionHandler() {
            let $vue = this;
            axios_.post("/academico/tramiteacademico/docenteResolucion/anular", $vue.docenteResolucion)
                .then(response => {
